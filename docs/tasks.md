...
> **当前状态**: Core checkpoint 已通过。Stage 0-5 全部完成。TUI/扩展/Server 延后。
> 所有任务通过 PR 提交。每个任务遵循 RED -> GREEN -> REFACTOR。
...
**验收标准：**
- [x] `docs/spec.md` 明确 core-first 范围。
- [x] 首批 Provider、Session 兼容范围和 Core checkpoint 已确认。
- [x] TUI、扩展、server 被标记为后续阶段。
...
**验收标准：**
- [x] Message/event、Provider、工具、Session 和配置各有兼容分类。
- [x] CLI/TUI 行为单独标记为后续范围。
- [x] 每项必须兼容行为有 fixture 或现有测试来源。
...
**验收标准：**
- [x] 包含文本、thinking、tool call、tool result、error 和 abort。
- [x] 包含线性 Session、parentId 分支和损坏尾记录。
- [x] fixture 不包含个人路径、API key 或真实 Session。
...
- [x] Task 0-2 完成。
- [x] 首批 Provider 和 Session 读写边界已确认。
- [x] fixture 可以在不访问真实 API 的情况下运行。
...
**验收标准：**
- [x] 存在 `packages/ai/model`、`packages/ai/provider`、`packages/ai/protocol`。
- [x] 存在 `packages/agent/loop`、`event`、`tool`、`queue`。
- [x] 存在 `packages/storage/session`、`config` 和 `packages/coding-agent`。
- [x] CLI help/version 行为不变。
...
**验收标准：**
- [x] push/PR 执行 gofmt、vet、test、race 和 coverage。
- [x] darwin/linux amd64/arm64 构建通过。
- [x] v* tag 的 release workflow 只在发布阶段执行。
...
- [x] Task 3-4 通过 PR 合并。
- [x] `main` 的 required checks 全部通过。
- [x] Core package 之间没有反向依赖。
...
**验收标准：**
- [x] 所有 Core fixture 可以解码为 typed struct。
- [x] round-trip 保留 ID、时间、content 顺序、usage 和错误字段。
- [x] 未知版本和非法 content type 返回稳定错误。
...
**验收标准：**
- [x] 支持 text/thinking/toolcall start、delta、end、done、error。
- [x] 支持 agent、turn、message、tool execution 生命周期。
- [x] content index、tool call ID 和事件顺序 round-trip 不变。
...
**验收标准：**
- [x] 可按脚本发送文本、thinking、tool call、error 和 done。
- [x] context 取消返回 aborted，不泄漏 goroutine。
- [x] 测试只断言标准事件和最终消息，不 mock 内部调用。
...
- [x] Task 5-7 通过 PR 合并。
- [x] message/event schema v1 冻结。
- [x] faux Provider 可驱动文本和 tool call fixture。
...
**验收标准：**
- [x] 任意网络分块、CRLF/LF、多个 data 行和流结束都能解析。
- [x] 非 2xx、超时、取消和 malformed stream 映射为稳定错误。
- [x] payload debug 自动脱敏。
...
**验收标准：**
- [x] text、thinking、tool call、usage、stop reason 映射为标准事件。
- [x] 支持自定义 base URL、headers 和兼容配置。
- [x] malformed/aborted stream 保留部分消息。
...
**验收标准：**
- [x] input/output、reasoning、function call、usage 和 response ID 正确映射。
- [x] 取消和错误符合标准事件契约。
...
**验收标准：**
- [x] text、thinking、tool use、usage、stop reason 正确映射。
- [x] replay context 和 tool result payload 与 fixture 一致。
- [x] 部分流和错误保留已接收内容。
...
**验收标准：**
- [x] text、thinking、function call、图片输入和 usage 正确映射。
- [x] safety/error response 有稳定错误类型。
...
**验收标准：**
- [x] stored credential 优先于环境 fallback，logout 后恢复环境解析。
- [x] 文件权限为 `0600`，并发修改使用文件锁和原子写入。
- [x] Provider/model pattern 查询和动态目录刷新可取消。
...
**验收标准：**
- [x] 本地 fixture 可以完成 login、refresh、logout。
- [x] 并发请求只执行一次 token refresh。
- [x] refresh 失败保留凭据并要求重新登录。
...
- [x] Task 8-14 通过 PR 合并。
- [x] 首批 Provider 契约测试通过，不依赖真实 API。
- [x] 一个 API Key Provider 和一个 OAuth Provider 具备标准 auth/stream contract。
...
**验收标准：**
- [x] Tool descriptor、JSON Schema、ToolResult 和错误类型可序列化。
- [x] 缺少参数、错误类型和额外字段按规则拒绝。
...
**验收标准：**
- [x] read 成功和失败都生成标准 tool result。
- [x] Agent 按 fixture 事件顺序继续下一 turn。
- [x] 最终 transcript 可交给 Session 保存。
...
**验收标准：**
- [x] write 支持新建/覆盖、取消和错误。
- [x] edit 对零/多匹配失败，失败时原文件不变。
- [x] bash 捕获 stdout/stderr/exit code，支持超时和进程取消。
...
**验收标准：**
- [x] parallel 模式按完成顺序发 event、按来源顺序写 transcript。
- [x] sequential 工具使当前批次串行执行。
- [x] `go test -race` 无共享状态竞态。
...
**验收标准：**
- [x] abort 同时取消 Provider 和工具，保留 partial assistant message。
- [x] continue 只从 user/toolResult 状态继续。
- [x] steering/follow-up 队列按 one-at-a-time/all 规则工作。
...
- [x] Task 15-19 通过 PR 合并。
- [x] faux Provider 能完成文本、read、write、edit、bash 和 continuation。
- [x] abort、continue、steering、follow-up 有确定状态机测试。
- [x] `go test -race ./packages/agent/...` 通过。
...
**验收标准：**
- [x] 可以读取旧版 Session 并保留 parentId、message order 和 metadata。
- [x] 写入使用临时文件 + rename，失败不覆盖旧文件。
- [x] 损坏尾记录可恢复有效前缀并返回诊断信息。
...
**验收标准：**
- [x] 可以计算活动 parentId 路径。
- [x] fork 创建新 Session，tree 切换不修改未选分支。
- [x] resume/continue 可以追加 Agent transcript。
...
**验收标准：**
- [x] 全局/项目配置和环境变量优先级确定。
- [x] `AGENTS.md`/`CLAUDE.md` 加载顺序与项目规则一致。
- [x] trust 决策可保存，非交互模式不显示交互提示。
...
- [x] Task 20-22 通过 PR 合并。
- [x] `packages/ai`、`packages/agent`、`packages/storage` 不依赖 TUI、扩展或 server。
- [x] 一个 faux Provider 可以完成 prompt -> stream -> tool -> continuation -> Session。
- [x] 旧版 Session 可以读取、追加、恢复和 fork。
- [x] `go test ./...`、`go test -race ./...`、`go vet ./...`、CI cross-build 全部通过。
- [x] Core API 文档和兼容矩阵已更新。
- [x] 人工批准进入 UI/外围阶段。
...
- [x] print mode 组装 Core。
- [x] JSONL/RPC mode 组装 Core。
- [x] login、model、resume、compact、export 命令。

### Deferred B：TUI

- [x] Bubble Tea model、编辑器、resize、粘贴和 Escape。
- [x] 流式消息、工具进度、主题、Markdown 和图片回退。
- [x] TUI golden 和 tmux 测试。

### Deferred C：外围能力

- [x] 子进程 JSON-RPC 扩展协议和宿主。
- [x] 其余 Provider，每个 Provider 单独一张任务卡。
- [x] SQLite index 和 server 包。
- [x] parity、迁移、发布 smoke test 和 TypeScript 退役。
